...
# Introduction
The aim of this report is to explore the relationships between bike usage, weather, time of day and holidays in rental bike sharing systems using data from Seoul, South Korea and Washington DC, USA.

The datasets used contain the number of bikes rented every hour in each city, together with other variables.
...
#    So I couldn't use the same x-axis and only retain month data.
...
The figure below shows the change in air temperature over time in Seoul and Washington DC.

Since there are fluctuations in air temperature over the course of each day, the average daily temperature was calculated prior to creating the plots.
...
The figure below shows the average number of rented bikes per hour for each season in both cities, with error bars representing 95% confidence intervals.
...
The figure below shows the average number of rented bikes per hour against whether they were rented on a holiday, with error bars representing 95% confidence intervals.
...
The figure below shows the average number of rented bikes per hour against time of day, with error bars representing 95% confidence intervals.
...
In Seoul, we see a steadier increase in bike demand between 10am and 6pm. In Washington DC, we see bike demand decreasing from 8am to 10am, then a small statistically insignificant peak around lunchtime 12noon and 1pm (error bars overlapping).
...
When fitting linear models, we make the following assumptions:

1. __Normality__: assume the errors are well described by a Normal distribution with mean 0.
2. __Constant Error Variance__: variance is constant and not related to the mean of the fitted values (or anything else)
3. __Residual Independence__: we assume errors are independent of each other
4. __Linearity__: X is permitted to increase or decrecrease linearly with Y.

A linear model was fitted for each city with log count as outcome, and season, air temperature, humidity and wind speed as predictors.
...
Looking at Figures __Histogram of Residuals__ and __Normal Q-Q Plot__, we can see the distribution is left-skewed. This indicates non-normality.
